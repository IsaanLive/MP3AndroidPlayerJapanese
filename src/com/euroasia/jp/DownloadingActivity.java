...
public class DownloadingActivity extends ListActivity {
    ...
    public void onCreate(Bundle icicle) {
        ...
        if(totaldownloading >= 1){
        	text.setText("全体ダウンロード: "+totaldownloading);
        }else{
        	text.setText("全体ダウンロード: 0");
        }
        ...
        this.clearall.setOnClickListener(new OnClickListener() {
        	public void onClick(View v) {
        		...
        		toastmake("再設定完了しました& ダウンロード失敗しました");
        	}
        });
		...
		public View getView(int position, View convertView, ViewGroup parent){
	    	...
	    	if(items.get(position).third == 100){
	    		...
	    		if(a == 2){
		    		...
		    		if(a == 1){
		        		progress.setText("ダウンロード待機中");
		    		}else if(a == 0){
		    			progress.setText("ダウンロード完了しました");
		    		}else if(a == -1){
		        		progress.setText("ダウンロード失敗しました");
		    		}
	    		}
	...
	protected void onListItemClick(ListView l, View v, int position, final long id) {
		...
		if(items.get(position).third != 100){
			...
			if(downloaded == -1){
				...
				builder.setMessage("ダウンロードオプション")
				       .setCancelable(false)
				       .setNegativeButton("削除", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				                mDbHelper.deleteDownload(ida);
				                refreshlist();
				                toastmake("MP3 ダウンロード　削除しました");
				           }
				       })
				       .setNeutralButton("やり直し", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				...
				final CharSequence[] items = {"MP3を開く", "着信音に設定", "再びダウンロード", "削除", getString(R.string.btn_cancel_name)};
	
				AlertDialog.Builder builder = new AlertDialog.Builder(this);
				builder.setTitle("オプションの選択");
				builder.setItems(items, new DialogInterface.OnClickListener() {
				    public void onClick(DialogInterface dialog, int item) {
				    	if(items[item] == "MP3を開く"){
				    	    try {
				    	        //launch intent
				    	    	...
				    	    	toastmake("MP3 ローディングエラー");
				    	        e.printStackTrace();
				    	    }
				    	}else if(items[item] == "着信音に設定"){
							...
							builder.setMessage("本当に削除しますか: " + title + " ?")
							       ...
							       .setPositiveButton("はい", new DialogInterface.OnClickListener() {
							           public void onClick(DialogInterface dialog, int id) {
							        	    ...
							        	    mDbHelper.deleteDownload(ida);
							        	    refreshlist();
							                toastmake("MP3 削除しました");
							           }
							       })
							       .setNegativeButton("いいえ", new DialogInterface.OnClickListener() {
							           public void onClick(DialogInterface dialog, int id) {
							        	   toastmake("MP3 削除しませんでした");
							               dialog.cancel();
							           }
							       });
							AlertDialog alert = builder.create();
							alert.show();
				    	}else if(items[item] == "再びダウンロード"){
				    		mDbHelper.updateDownloaded(ida, "1"); 
				    		refreshlist();
				    		toastmake("やり直しの設定");
				    	}
				...
				builder.setMessage("ダウンロードオプション")
				       .setCancelable(false)
				       .setNegativeButton("ダウンロード中止", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				        	   if(sid == 1){
    ...
    public void refreshlist(){
    	...
    	if(a.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102109, "現在ダウンロード中", 100));
    		a.moveToFirst();
    	...
    	if(a1.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102110, "ダウンロード待機中", 100));
    	...
    	if(a11.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102111, "ダウンロード完了しました", 100));
    	...
    	if(a111.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102112, "ダウンロード失敗しました", 100));
